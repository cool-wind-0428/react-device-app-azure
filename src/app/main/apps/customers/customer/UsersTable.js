...
import { getUsers, selectUsers } from '../store/customersSlice';
...
import { openUserProfileDialog, removeUsers } from '../store/customersSlice';
...
function UsersTable(props) {
	...
	const searchText = useSelector(({ customersApp }) => customersApp.users.searchText);
	...
	useEffect(() => {
		...
		// if(authUser.role !== 'admin')
			_users = _.filter(users, item => item.userId === authUser.uuid);
		if (searchText.length !== 0) {
			setData(_.filter(_users, item => item.displayName.toLowerCase().includes(searchText.toLowerCase())));
			setPage(0);
		} else {
			setData(_users);
		}
	}, [users, searchText, authUser]);
								...
								.map((n, index) => {								
									...
									return (
										...
										>
											...
											<TableCell className="p-2 md:p-2" component="th" scope="row" align="center">
												{
													_.map(_.filter(devices, device => n.devices.includes(device.id)), item => item.name).join(', ')													
												}
											</TableCell>
